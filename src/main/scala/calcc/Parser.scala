...
class Parser(table:  LRTable){
  def parse(input: List[Terminal]) = {
    def parser(input: List[Terminal], stack: List[Int],t: List[Tree[GrammarSymbol]]): Either[Tree[GrammarSymbol], String] = {
        ...
        case Some(x) => x._2 match{
          ...
          case Reduce(y) =>
            ...
            table._2.get((y.left, newStack.head)) match{
              case None => Right(s"No next state in the GOTO table, failed to parse ${input.head} at position ${input.head.pos}")
              case Some(z) =>
                val (treeHead, treeTail) = t.splitAt(y.right.size)
                parser(input, z::newStack, y.f(treeHead) :: treeTail)
            }
          case Accept => Left(t.head)
        }
      }
    }
