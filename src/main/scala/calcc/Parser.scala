  ...
  def parse(input: List[Terminal]) = {
    def parser(input: List[Terminal], stack: List[Int],t: List[Tree[GrammarSymbol]]): Tree[GrammarSymbol] = {
      ...
      table._1.filterKeys{ t => t._2 == stack.head && t._1.getClass == input.head.getClass }.headOption match{
        case None => throw new IllegalArgumentException(s"No next state in the action table, failed to parse ${input.head} at position ${input.head.pos}")
        case Some(x) => x._2 match{
          ...
          case Reduce(y) =>
            ...
            table._2.get((y.left, newStack.head)) match{
              case None => throw new IllegalArgumentException(s"No next state in the GOTO table, failed to parse ${input.head} at position ${input.head.pos}")
              case Some(z) =>
