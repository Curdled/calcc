...
class parserTest extends FunSuite with BeforeAndAfter {
  ...
  test("Parsing simple stuff") {
    //0
    assert(p.parse(List(LNum(List('0'), 0))) == Left(Node(LNum(List('0'), 0), List(Leaf))))

    //0!
    assert(p.parse(List(LNum(List('0'), 0), LFact(0))) == Left(Node(LFact(0), List(Node(LNum(List('0'), 0), List(Leaf))))))

    //Cos0
    assert(p.parse(List(LCos(0), LNum(List('0'),0))) == Left(Node(LCos(0), List(Node(LNum(List('0'), 0), List(Leaf))))))

    //0+1
    assert(p.parse(List(LNum(List('0'),0), LPlus(0), LNum(List('1'),0))) ==
      Left(Node(LPlus(0), List(Node(LNum(List('0'),0), List(Leaf)), Node(LNum(List('1'), 0), List(Leaf))))))

    //-1
    assert(p.parse(List(LMinus(0), LNum(List('0'),0))) ==
      Left(Node(LMinus(0), List(Node(LNum(List('0'),0), List(Leaf))))))
  }

  test("Simple expressions") {
    assert(p.parse(List(LNum(List('0'),0), LMinus(0), LNum(List('1'),0))) ==
      Left(Node(LMinus(0), List(Node(LNum(List('0'),0), List(Leaf)), Node(LNum(List('1'), 0), List(Leaf))))))
